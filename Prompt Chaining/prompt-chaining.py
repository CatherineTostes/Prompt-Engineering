...
spec_to_schema = PromptTemplate.from_template(
    ...
    From the following product spec, extract a minimal JSON Schema with fields and 
    types.
...
schema_to_routes = PromptTemplate.from_template(
    """You are a Go senior developer.
    Given the JSON schema below, design REST routes and sketch idiomatic Go handlers 
    for CRUD.
...
commit_message = PromptTemplate.from_template(
    ...
    Write a single-line conventional commit message summarizing the new API base on 
    schema and routes.
...
spec_text = """We need a Prducts API.
    Fields: id (uuid), name (string, required), description (string), price (float, > 0),
    stock (int, > 0).
...
print("Save result in prompt_chaining_result.md")