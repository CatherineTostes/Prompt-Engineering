...
msg1 = """
You are a senior backend engineer. A junior developer asked you how to optimize SQL queries for a better performance.
...
msg2 = f"""
You are a software architecture. I want you to produce an Architecture Decision Record (ADR) about choosing PostgreSQL instead of MongoDB
...
Keep the final ADR professional, structures and easy to read.
"""
...
The skeleton must cover: data model definition in Go (structs) choice of HTTP framework or net/http, 
routing, handlers, validations, database layer, error handling and project strutucture. Do not expand yet.

Step 2: Expand each bullet point with clear technical details, examples and go best practices.
Include sample code snippets in Go (structs, handlers, routes) and considerations about 
packages (e.g., chi or net/http), error handling with idiomatic Go, and how to organize the project into 
packages (handlers, models, db).

The API must implemented CRUD operations for products with fields: id, name, description, price, stock.
"""
